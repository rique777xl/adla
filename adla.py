...
class AutonomousGameEconomyAI:
    ...
    def _build_decision_model(self):
        """Modelo de decisão com aprendizado por reforço. 25 features."""
    ...
    def _build_critic_model(self):
        """Modelo crítico para avaliar decisões. 25 features."""
        state_input = Input(shape=(25,))
    ...
    def calculate_advanced_state(self, data, withdrawal_rate):
        ...
        # Definir divisores para normalização das 10 métricas
        divisors = {
        ...
        state.append(float(withdrawal_rate)) 
        ...
        for factor_key in factor_keys_to_include:
            state.append(self.factor_weights.get(factor_key, 1.0)) 
    ...
    def read_txt_data(self):
        ...
        try:
            ...
            try:
                ...
                print(f"Erro ao criar 'current_withdrawal_rate.txt': {e}")
            data['current_withdrawal_rate'] = 0.20 
    ...
    def _calculate_performance_score(self, data, rate):
        ...
        sentiment_value = self._map_sentiment_to_float(data.get('community_sentiment', 'neutral'))
        sentiment_ratio = (sentiment_value + 1) / 2 
    ...
    def calculate_reward(self, old_state, new_state, action):
        ...
        if len(old_state) > 6 and len(new_state) > 6:
            ...
            print("Aviso: Estados incompletos para cálculo de recompensa (preço).")
        
        
        if len(old_state) > 12 and len(new_state) > 12:
            old_churn = old_state[12]
            new_churn = new_state[12]
            if new_churn < old_churn: 
                reward += (old_churn - new_churn) * 5
            else: 
                reward -= (new_churn - old_churn) * 3
        else:
            print("Aviso: Estados incompletos para cálculo de recompensa (churn).")
        ...
        if len(new_state) > 0:
            ...
            print("Aviso: Estados incompletos para cálculo de recompensa (volume).")
        ...
        if len(new_state) > 10:
            ...
            print("Aviso: Estados incompletos para cálculo de recompensa (sentimento).")
        ...
        if len(new_state) > 18:
            ...
            print("Aviso: Estados incompletos para cálculo de recompensa (volatilidade).")
        ...
        if len(new_state) > 15:
            ...
            print("Aviso: Estados incompletos para cálculo de recompensa (liquidez).")
    ...
    def remember_current(self, data, state, rate, action):
        ...
        decision_data = {
            ...
            'rate': float(rate), # Ensure native float
            'action': float(action), # Ensure native float
            'state': state.tolist(), # Armazena como lista para serialização JSON
            'data': {k: (float(v) if isinstance(v, (int, float, np.floating)) else v) for k, v in data.items()}, # Ensure native floats
            'performance_score': float(self._calculate_performance_score(data, rate)) # Ensure native float
        }
    ...
    def _generate_analysis(self, data, state, rate):
        ...
        # Corrigindo a string japonesa para garantir compatibilidade com UTF-8
        if rate > 0.3: 
    ...
    def decide_action(self, state):
        ...
        return float(rate), float(action) # Ensure native floats are returned
    ...
    def process_decision(self, data):
        ...
        try:
            ...
            decision_data = {
                ...
                'state': state.tolist(), # Armazena como lista para serialização JSON
                'data': {k: (float(v) if isinstance(v, (int, float, np.floating)) else v) for k, v in data.items()}, # Ensure native floats
                'performance_score': float(performance_score) # Ensure native float
            }
            self.decision_history.append(decision_data)
            self.performance_history.append(decision_data)
            ...
            return {"rate": float(rate), "analysis": full_analysis, "state": state, "action": float(action)}
        except Exception as e:
    ...
    def _save_decision(self, rate, analysis):
        ...
        # Armazena taxa atual
        with open('current_withdrawal_rate.txt', 'w', encoding='utf-8') as f: # Adicionado encoding
            f.write(f"{float(rate):.6f}") # Ensure float

        # Registra histórico de decisões
        try:
            with open('ai_decisions_log.json', 'a', encoding='utf-8') as f: # Adicionado encoding
                f.write(json.dumps({
                    'timestamp': datetime.now().isoformat(),
                    'rate': float(rate), # Ensure float
                    'analysis': analysis
                }, ensure_ascii=False) + '\n') # ensure_ascii=False para log, sem erro no arquivo
        except Exception as e:
    ...
    def _save_rewards(self):
        ...
        try:
            # Salva a memória completa.
            # Convertendo tuplas para listas e np.ndarray para list e np.float32 para float
            with open('ai_rewards_log.json', 'w', encoding='utf-8') as f: # Adicionado encoding
                json.dump([
                    [s.tolist(), float(a), float(r), ns.tolist() if ns is not None else None] 
                    for s, a, r, ns in self.memory
                ], f, indent=4, ensure_ascii=False) # ensure_ascii=False para o log
    ...
    def _save_state_snapshot(self, data, state, rate, action):
        ...
        snapshot = {
            ...
            'rate': float(rate),
            'action': float(action), # Salva a ação aqui
        ...
        try:
            with open('state_snapshots.json', 'a', encoding='utf-8') as f: # Adicionado encoding
                ...
                try:
                    ...
                    if next_state_np is not None:
                        ...
                        next_q = self.critic_model.predict(
                            [next_state_np.reshape(1, -1), np.array([[float(next_action)]], dtype=np.float32)], # Ensure float
                            verbose=0
                        )[0][0]
                        target = float(reward) + 0.95 * float(next_q)  # Γ = 0.95, Ensure floats
                    else:
                        target = float(reward) # Ensure float
                        
                    # Treina critic model
                    self.critic_model.fit(
                        [state_np.reshape(1, -1), np.array([[float(action)]], dtype=np.float32)], # Ensure float
                        np.array([[float(target)]], dtype=np.float32), # Ensure float
                    ...
                    self.decision_model.fit(
                        ...
                        np.array([[float(action)]], dtype=np.float32), # Ensure float
                        epochs=1, verbose=0, batch_size=1
                    )
    ...
    def train_on_historical_data(self):
        ...
        if os.path.exists('state_snapshots.json'):
            try:
                with open('state_snapshots.json', 'r', encoding='utf-8') as f: # Adicionado encoding
                    for line in f:
                        try:
        ...
        for i in range(len(historical_data) - 1):
            try:
                ...
                action = float(current_snapshot.get('action', 0.5)) # Use .get() for robustness, ensure float
    ...
    def save_models(self):
        ...
        ai_config = {
            ...
            # Converte deque para lista de LISTAS para serialização JSON
            'current_memory': [
                [s.tolist(), float(a), float(r), ns.tolist() if ns is not None else None] 
                for s, a, r, ns in self.memory
            ], 
        ...
        try:
            with open('ai_config.json', 'w', encoding='utf-8') as f: # Adicionado encoding
                json.dump(ai_config, f, indent=4, ensure_ascii=False) # ensure_ascii=False
    ...
    def load_config(self):
        ...
        try:
            with open('ai_config.json', 'r', encoding='utf-8') as f: # Adicionado encoding
                config = json.load(f)
            
            self.epsilon = config.get('epsilon', 1.0)
            self.factor_weights = config.get('factor_weights', self.factor_weights)
            
            # Recria a memória, convertendo de volta para np.array e garantindo floats nativos
            self.memory = deque(maxlen=10000)
            for item in config.get('current_memory', []):
                s, a, r, ns = item # Agora item é uma lista (não uma tupla)
                action_val = float(a)
                reward_val = float(r)
            ...
            for entry in config.get('decision_history', []):
                ...
                # Garante que 'rate', 'action', 'performance_score' sejam floats nativos ao carregar
                ...
                # Também converte os valores em 'data' para float se forem numéricos
...
def create_sample_files():
    ...
    for filename, content in basic_data.items():
        try:
            if not os.path.exists(filename):
                with open(filename, 'w', encoding='utf-8') as f: # Adicionado encoding
                    f.write(content)
...
def execute_phase1():
    ...
    # Certifique-se de que a saída do console pode lidar com UTF-8 (chcp 65001)
...
def load_reward_history(): # Esta função não está sendo usada no seu __main__
    """Carrega histórico de recompensas para análise (agora lê a memória completa)."""
    try:
        if os.path.exists('ai_rewards_log.json'):
            with open('ai_rewards_log.json', 'r', encoding='utf-8') as f: # Adicionado encoding
                # Ao carregar, converte os elementos numéricos de volta para numpy array se necessário
                ...
                for item in loaded_data:
                    ...
                    state_np = np.array(s, dtype=np.float32)
                    next_state_np = np.array(ns, dtype=np.float32) if ns is not None else None
                    reconstructed_memory.append((state_np, action_val, reward_val, next_state_np))
...
def update_weights(reward_data): # Esta função não está sendo usada no seu __main__
    """Atualiza pesos dos fatores (chamada para adapt_weights)."""
    print("Atualizando pesos dos fatores (se houver dados suficientes)...")
    # Aparentemente, 'adapt_weights' não está definido na sua classe AutonomousGameEconomyAI
    # Se você tem essa função, ela precisaria ser um método da classe ou ser definida aqui.
    # Por ora, esta função está incompleta ou aponta para um método que não existe.
    # ai.adapt_weights(ai.performance_history) # Pass performance history to it

def get_metric_correlation(rewards, metric): # Esta função não está sendo usada no seu __main__
...
def adjust_weight_based_on_correlation(correlation): # Esta função não está sendo usada no seu __main__
...
def ai_performance_monitoring():
    ...
    try:
        # Lendo com encoding para evitar UnicodeDecodeError ao carregar o log
        with open('ai_decisions_log.json', 'r', encoding='utf-8') as f:
    ...
    if len(decisions) >= 10:
        ...
        # Corrigindo a mensagem em japonês aqui
...
def update_trust_confidence(confidence_level):
    ...
    try:
        with open('trust.log', 'a', encoding='utf-8') as f: # Adicionado encoding
...
def create_stability_report(decisions):
    ...
    try:
        # Adicionado encoding='utf-8' e ensure_ascii=False para o arquivo de log
        with open(stability_data['filename'], 'a', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    # --- Chamar script para enviar o hash para a blockchain polygon---
    print("\n--- Enviando hash do estado da IA para a Blockchain ---")
    try:
        # 
        # 
        os.system("python blockchain_publisher.py") 
        print("Solicitação de registro na blockchain enviada.")
    except Exception as e:
        print(f"Erro ao chamar blockchain_publisher.py: {e}")
